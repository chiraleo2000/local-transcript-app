    ...
    for dtype in asr_cuda_dtypes_to_try(torch):
        for attempt in range(2):
            try:
                ...
                if oom_fallback:
                    logger.warning(
                        "Pathumma CUDA OOM with %s; falling back to float16.",
                        dtype,
                    )
                    recover_cuda()
                    break
                if attempt == 0 and is_cuda_recoverable(exc):
                    logger.warning(
                        "Pathumma CUDA load failed (%s); recovering and retrying.",
                        exc,
                    )
                    recover_cuda()
                    continue
                raise
    raise last_exc or RuntimeError("Pathumma CUDA load failed after retry")
...
def _reload_cuda_pipeline():
    ...
    logger.info("Pathumma Whisper CUDA pipeline reloaded.")
    return pipe


def _load_cuda_pipeline(hf_token: str | None, dtype=None):
    """Build Pathumma pipeline on NVIDIA CUDA (FP32 on P4, FP16 on Ampere+)."""
    import torch
    ...
    logger.info("Using CUDA (%s) backend for Pathumma Whisper.", dtype)
